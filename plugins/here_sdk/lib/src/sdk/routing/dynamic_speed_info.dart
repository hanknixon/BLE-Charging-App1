...
class DynamicSpeedInfo {
  ...
  /// Specifically, 0.0 means free traffic and 10.0 means stationary traffic.
  ///
  ...
  /// @nodoc
  @visibleForTesting
  static dynamic $prototype = DynamicSpeedInfo$Impl();
}
...
/// @nodoc
@visibleForTesting
class DynamicSpeedInfo$Impl {
  double calculateJamFactor(DynamicSpeedInfo $that) {
    final _calculateJamFactorFfi = __lib.catchArgumentError(() => __lib.nativeLibrary.lookupFunction<Double Function(Pointer<Void>, Int32), double Function(Pointer<Void>, int)>('here_sdk_sdk_routing_DynamicSpeedInfo_calculateJamFactor'));
    final _handle = sdkRoutingDynamicspeedinfoToFfi($that);
    final __resultHandle = _calculateJamFactorFfi(_handle, __lib.LibraryContext.isolateId);
