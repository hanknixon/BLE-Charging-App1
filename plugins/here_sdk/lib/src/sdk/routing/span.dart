...
abstract class Span implements Finalizable {
  ...
  /// The list of indexes to [Section.sectionNotices] the parent section owns.
  ...
  /// The road numbers on the span enriched with information specific to _route numbers_
  ...
  /// The list of indexes to [Section.noThroughRestrictions] the parent section owns.
  /// In case the list is not empty, the user must judge all the indexed sdk routing noThroughRestriction's
  /// carefully before proceeding.
  /// Get the list of indexes to [Section.noThroughRestrictions] the parent section owns.
  /// In case the list is not empty, the user must judge all the indexed [Section.noThroughRestrictions]'s
  /// carefully before proceeding.
...
class Span$Impl extends __lib.NativeBase implements Span {
  ...
  @override
  List<int> get noThroughRestrictionsIndexes {
    final _getFfi = __lib.catchArgumentError(() => __lib.nativeLibrary.lookupFunction<Pointer<Void> Function(Pointer<Void>, Int32), Pointer<Void> Function(Pointer<Void>, int)>('here_sdk_sdk_routing_Span_noThroughRestrictionsIndexes_get'));
    final _handle = this.handle;
    final __resultHandle = _getFfi(_handle, __lib.LibraryContext.isolateId);
    try {
      return heresdkRoutingCommonBindingslistofIntFromFfi(__resultHandle);
